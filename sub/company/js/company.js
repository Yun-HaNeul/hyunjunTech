function secAniHandler(){
    
    window.addEventListener("scroll", objAniHandler);

    function objAniHandler(){
        const section = document.querySelectorAll(".section");
        const obj1 = document.querySelectorAll(".box");

        section.forEach(function(item){
            // console.log(item);
            let sct = item.offsetTop - window.innerHeight / 2;
        ...
        obj1.forEach(function(item){
            ...
            if(window.pageYOffset > item.offsetTop - 500){
                item.classList.add("active");
            }else {
